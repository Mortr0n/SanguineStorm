...
public class MapFunctions
{
	...
	public static int[,] PerlinNoise(int[,] map, float seed)
    {
        ...
        for (var x = 0; x < map.GetUpperBound(0); x++)
        {
            ...
            for (var y = newPoint; y >= 0; y--) map[x, y] = 1;
        }
	...
	public static int[,] PerlinNoiseSmooth(int[,] map, float seed, int interval)
    {
        ...
        if (interval > 1)
        {
            ...
            for (var i = 1; i < points; i++)
            {
                ...
                {
                    for (var y = Mathf.FloorToInt(currHeight); y > 0; y--) map[x, y] = 1;
	...
	public static int[,] RandomWalkCave(int[,] map, float seed, int requiredFloorPercent)
    {
        ...
        var floorX = rand.Next(1, map.GetUpperBound(0) - 1);
        //Define our start y position
        var floorY = rand.Next(1, map.GetUpperBound(1) - 1);
        //Determine our required floorAmount
        var reqFloorAmount = map.GetUpperBound(1) * map.GetUpperBound(0) * requiredFloorPercent / 100;
	...
	public static int[,] RandomWalkCaveCustom(int[,] map, float seed, int requiredFloorPercent)
    {
        ...
        //Define our start x position
        var floorX = UnityEngine.Random.Range(1, map.GetUpperBound(0) - 1);
        //Define our start y position
        var floorY = UnityEngine.Random.Range(1, map.GetUpperBound(1) - 1);
        //Determine our required floorAmount
        var reqFloorAmount = map.GetUpperBound(1) * map.GetUpperBound(0) * requiredFloorPercent / 100;
        //Used for our while loop, when this reaches our reqFloorAmount we will stop tunneling
        var floorCount = 0;
	...
	public static int[,] DirectionalTunnel(int[,] map, int minPathWidth, int maxPathWidth, int maxPathChange,
        int roughness, int windyness)
    {
        ...
        var x = map.GetUpperBound(0) / 2;

        //Set up our seed for the random.
        var rand = new Random(Time.time.GetHashCode());

        //Create the first part of the tunnel
        for (var i = -tunnelWidth; i <= tunnelWidth; i++) map[x + i, 0] = 0;

        //Cycle through the array
        for (var y = 1; y < map.GetUpperBound(1); y++)
        {
            //Check if we can change the roughness
            if (rand.Next(0, 100) > roughness)
            {
                //Get the amount we will change for the width
                var widthChange = UnityEngine.Random.Range(-maxPathWidth, maxPathWidth);
                tunnelWidth += widthChange;

                //Check to see we arent making the path too small
                if (tunnelWidth < minPathWidth) tunnelWidth = minPathWidth;

                //Check that the path width isnt over our maximum
                if (tunnelWidth > maxPathWidth) tunnelWidth = maxPathWidth;
            }
            ...
            if (rand.Next(0, 100) > windyness)
            {
                ...
                var xChange = UnityEngine.Random.Range(-maxPathChange, maxPathChange);
                x += xChange;

                //Check we arent too close to the left side of the map
                if (x < maxPathWidth) x = maxPathWidth;
                //Check we arent too close to the right side of the map
                if (x > map.GetUpperBound(0) - maxPathWidth) x = map.GetUpperBound(0) - maxPathWidth;
            }

            //Work through the width of the tunnel
            for (var i = -tunnelWidth; i <= tunnelWidth; i++) map[x + i, y] = 0;
        }
	...
	public static int[,] SmoothVNCellularAutomata(int[,] map, bool edgesAreWalls, int smoothCount)
    {
        ...
        for (var y = 0; y < map.GetUpperBound(1); y++)
        {
            ...
            var surroundingTiles = GetVNSurroundingTiles(map, x, y, edgesAreWalls);
            ...
            else if (surroundingTiles < 2) map[x, y] = 0;
	...
	/// <summary>
	///     Gets the surrounding tiles using the von Neumann Neighbourhood rules. This neighbourhood only checks the direct
	///     neighbours, i.e. Up, Left, Down Right
	/// </summary>
	...
	private static int GetVNSurroundingTiles(int[,] map, int x, int y, bool edgesAreWalls)
    {
        ...
        if (x - 1 > 0)
            tileCount += map[x - 1, y];
        else if (edgesAreWalls) tileCount++;
	...
	/// <summary>
	///     Smoothes a map using Moore's Neighbourhood Rules. Moores Neighbourhood consists of all neighbours of the tile,
	...
	public static int[,] SmoothMooreCellularAutomata(int[,] map, bool edgesAreWalls, int smoothCount)
    {
        ...
        for (var y = 0; y < map.GetUpperBound(1); y++)
        {
            var surroundingTiles = GetMooreSurroundingTiles(map, x, y, edgesAreWalls);
            ...
            else if (surroundingTiles < 4) map[x, y] = 0;
	...
	private static int GetMooreSurroundingTiles(int[,] map, int x, int y, bool edgesAreWalls)
    {
        ...
        for (var neighbourY = y - 1; neighbourY <= y + 1; neighbourY++)
            if (neighbourX >= 0 && neighbourX < map.GetUpperBound(0) && neighbourY >= 0 &&
                neighbourY < map.GetUpperBound(1))
