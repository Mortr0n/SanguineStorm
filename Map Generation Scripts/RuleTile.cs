...
namespace ProGen.Global_Scripts
{
    [Serializable]
    [CreateAssetMenu]
    public class RuleTile : TileBase
    {
        ...
        public bool RuleMatches(TilingRule rule, ref TileBase[] neighboringTiles, int angle)
        {
            for (var i = 0; i < NeighborCount; ++i)
            {
                ...
                if ((rule.m_Neighbors[i] == TilingRule.Neighbor.This && tile != this) ||
                    (rule.m_Neighbors[i] == TilingRule.Neighbor.NotThis && tile == this)) return false;
            }
        ...
        {
            if (mirrorX && mirrorY) return RotatedOrMirroredIndexes[1, original];
            if (mirrorX) return RotatedOrMirroredIndexes[3, original];
            if (mirrorY) return RotatedOrMirroredIndexes[4, original];
            return original;
        }
